...
│   ├── mquant_bill.yaml       (12KB)
│   ├── mvalidator_rachel.yaml (11KB)
│   └── mcurator_stewart.yaml  (7KB)
├── workflows/                 # 워크플로우 파일
│   └── adaptive_workflow_v5.yaml (48KB) # MOwner 역할 포함
└── UMIS_ChatGPT_Guide.md     # 본 가이드
...
- `adaptive_workflow_v5.yaml` - 전체 워크플로우
- `manalyst_albert.yaml` - 핵심 관찰 에이전트
- `mexplorer_steve.yaml` - 기회 해석 에이전트
...
```
필수: workflow + albert + steve + bill
선택: rachel (데이터 검증 필요시)
...
### Q: 전체 맥락을 잃어버린 것 같아요
A: `adaptive_workflow_v5.yaml`을 기준으로 현재 Stage를 확인하고 진행하세요.

### Q: 어떤 파일을 추가해야 할지 모르겠어요
A: 기본 3개(workflow + albert + steve)로 시작하고, 필요에 따라 추가하세요.
