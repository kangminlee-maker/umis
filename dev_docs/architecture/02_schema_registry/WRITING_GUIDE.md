# config/schema_registry.yaml 작성 가이드

**목적:** Schema Registry가 무엇을 담아야 하는지 명확히 하기

---

## 🎯 config/schema_registry.yaml이란?

### 역할

```yaml
config/schema_registry.yaml = "RAG 시스템의 헌법"
...
**비유:**
```yaml
config/schema_registry.yaml = 건축 도면
  • 모든 방(Layer)의 구조
  • 전기/수도(필드) 배치
  • 방 간 연결(매핑)
  • 안전 기준(검증)
...
## 🎯 config/schema_registry.yaml 구조
...
### config/schema_registry.yaml 작성 전 확인
...
```yaml
다음 단계:
  1. config/schema_registry.yaml 생성
