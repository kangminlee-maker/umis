...
#### 1. config/projection_rules.yaml 작성

```yaml
# config/projection_rules.yaml (신규)
...
```yaml
Day 3-4:
  ✅ config/projection_rules.yaml (초기 규칙)
  ✅ umis_rag/projection/hybrid_projector.py
  ✅ data/llm_projection_log.jsonl (로그)
  ✅ 테스트 통과
```
...
5. config/projection_rules.yaml
   → 규칙 기반 (90%)

6. data/llm_projection_log.jsonl
   → LLM 판단 로그
