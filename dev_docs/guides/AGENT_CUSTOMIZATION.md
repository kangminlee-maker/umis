...
**대상:** 모든 UMIS 사용자  
**방법:** config/agent_names.yaml 파일 수정  
**적용:** Cursor에서 즉시
...
**변경:**
```
Cursor에서 config/agent_names.yaml 수정
→ Cmd+S
→ 다음 분석부터 적용!
```
...
```yaml
기본 (config/agent_names.yaml 안 건드림):
  Observer, Explorer, Quantifier, Validator, Guardian
  
  → 공식적, 명확함

커스텀 (config/agent_names.yaml 수정):
  자기 이름, 한국어, 봇 이름 등
