...
**버전**: v7.5.0
**배포 일시**: 2025-11-10
**상태**: ✅ **Production Ready - 역할 분리 완성**
**아키텍처**: 6-Agent System (MECE) + 3-Tier Complete + 100% Coverage
...
## 🆕 v7.5.0 신규 기능 (2025-11-10 최신)

### Estimator/Quantifier 역할 분리 (MECE) + Tier 강화

**핵심**: "MECE 달성 (Estimator = 추정, Quantifier = 계산)"

#### 1. Estimator/Quantifier 역할 분리

```yaml
Before (v7.4.0):
  Estimator: 추정 + 비즈니스 계산 공식 (12개 템플릿)
  Quantifier: 계산 방법론
  → 중복 70-80% (MECE 위배)

After (v7.5.0):
  Estimator: 순수 값 추정만
    - "B2B SaaS ARPU는?" → 80,000원
    - 2,281줄 (-1,907줄, 46% 감소)
  
  Quantifier: 순수 계산만
    - LTV = ARPU / Churn (공식 적용)
    - 31개 계산 방법론
  
  → 중복 0% (MECE 달성)
```

#### 2. Tier 1/2 임계값 강화 (Tier 3 집중)
...
효과:
  - Tier 3로 넘어가는 비율 증가
  - Tier 3 = Estimator의 핵심!
  - Fermi 분해 활용도 증가
```

#### 3. Context 전달 개선 (재귀 시)

```python
# v7.5.0: _build_contextualized_question()

Before: "arpu는?" (❌ 애매)
After:  "B2B SaaS 한국 2024 시장의 ARPU는?" (✅ 구체적)
...
이유:
  - Estimator Tier 2와 70-80% 중복
  - Production 미사용
  - 유지보수 부담

대체:
  - Estimator Tier 2 (11 Sources)가 더 강력
  - 10개 신호 중 8개 커버
  - + DefiniteData, WebSearch 추가
```
