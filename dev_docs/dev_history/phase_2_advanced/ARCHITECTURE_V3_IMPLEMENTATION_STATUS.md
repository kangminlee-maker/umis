...
파일:
  ...
  • config/pattern_relationships.yaml (45개 관계)
...
파일:
  • config/schema_registry.yaml (PART 2: ID & Lineage)
...
구현됨:
  ✅ config/schema_registry.yaml에 메타데이터 정의
...
정의만:
  ✅ config/schema_registry.yaml PART 7
...
2. Routing YAML (2시간)
   파일: config/routing_policy.yaml, workflow_executor.py
   효과: 가독성, 유지보수
...
4. Fail-Safe Tier 2-3 (1일)
   파일: config/runtime.yaml, circuit_breaker.py
   효과: 운영 안정성
