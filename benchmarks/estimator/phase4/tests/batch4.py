#!/usr/bin/env python3
"""
Phase 4 Batch 4: Medium Effort 테스트 (필요 모델만)
- gpt-5.1 (reasoning_effort='medium')
- o4-mini (reasoning_effort='medium')

⚠️ 제외된 모델 (Batch 3과 동일한 결과):
- gpt-5-pro (high 고정)
- o1-pro (high 고정)
    ...
    try:
        ...
        if config['reasoning_effort_support']:
            ...
            if 'reasoning_effort_fixed' in config:
                print(f"    ⚠️ 이 모델은 '{config['reasoning_effort_fixed']}' 고정")
...
def save_partial_results(all_results, scenarios, test_config, error_info=None):
    ...
    output_file = f"phase4_batch4_medium_{status.lower()}_{timestamp}.json"

    data = {
        'metadata': {
            'timestamp': datetime.now().isoformat(),
            'batch': 'Batch 4 - Pro/Premium (Medium Effort)',
            'reasoning_effort': 'medium (일부 모델 high 고정)',
            'status': status,
...
def run_batch4_test():
    """Batch 4 테스트 실행 - Medium Effort (필요 모델만)"""
    print("=" * 120)
    print("Phase 4 Batch 4 - Medium Effort 테스트")
    print("=" * 120)
    print("✅ 테스트 모델: gpt-5.1, o4-mini (reasoning_effort='medium')")
    print("❌ 제외 모델: gpt-5-pro, o1-pro 계열 (effort='high' 고정, Batch 3과 동일)")
    print()

    client = OpenAI()

    # Batch 4: effort='medium' 적용 가능한 모델만
    test_config = [
        {'model': 'gpt-5.1', 'effort': 'medium', 'tier': 'Batch 4'},
        {'model': 'o4-mini', 'effort': 'medium', 'tier': 'Batch 4'},
    ]
    ...
    print("\n" + "=" * 120)
    if error_occurred:
        print(f"⚠️  Batch 4 부분 결과 ({len(all_results)}개 테스트 완료)")
    else:
        print("🏆 Batch 4 최종 순위 (3개 문제 평균)")
    print("=" * 120)
    print()
