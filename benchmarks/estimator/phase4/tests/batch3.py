#!/usr/bin/env python3
"""
Phase 4 Batch 3: Pro/Premium 모델 테스트
- gpt-5.1 (우선순위 1)
- gpt-5-pro (우선순위 2)
- o4-mini (우선순위 3)
- o1-pro (우선순위 4)
- o1-pro-2025-03-19 (우선순위 5)
...
def save_partial_results(all_results, scenarios, test_config, error_info=None):
    ...
    output_file = f"phase4_batch3_{status.lower()}_{timestamp}.json"

    data = {
        'metadata': {
            'timestamp': datetime.now().isoformat(),
            'batch': 'Batch 3 - Pro/Premium 모델',
            'status': status,
...
def run_batch3_test():
    """Batch 3 테스트 실행"""
    print("=" * 120)
    print("Phase 4 Batch 3 - Pro/Premium 모델 테스트")
    print("=" * 120)
    print()

    client = OpenAI()

    # Batch 3: Pro/Premium 모델 5개 (우선순위 재정렬)
    test_config = [
        {'model': 'gpt-5.1', 'effort': 'high', 'tier': 'Batch 3'},
        {'model': 'gpt-5-pro', 'effort': 'high', 'tier': 'Batch 3'},
        {'model': 'o4-mini', 'effort': 'high', 'tier': 'Batch 3'},
        {'model': 'o1-pro', 'effort': 'high', 'tier': 'Batch 3'},
        {'model': 'o1-pro-2025-03-19', 'effort': 'high', 'tier': 'Batch 3'},
    ]
    ...
    print("\n" + "=" * 120)
    if error_occurred:
        print(f"⚠️  Batch 3 부분 결과 ({len(all_results)}개 테스트 완료)")
    else:
        print("🏆 Batch 3 최종 순위 (3개 문제 평균)")
    print("=" * 120)
    print()
