...
### 출처 2: 보편적 지식 (Universal Knowledge)
```
보편적으로 알려진 지식:
  - 한국 인구: 5,000만
  - 서울 인구: 1,000만
...
🤖 AI 전략:
  - 웹 서치: "한국 인구 2024", "서울 버스 요금"
  - 일반 지식: 학습 데이터의 통계/공개 정보
  - 최신 데이터: 웹 서치로 보완
```

### 출처 3: 경험 패턴 (Experience Patterns)
```
경험적으로 알려진 패턴:
  - 식사 시간: 30분
  - 테이블당 인원: 3명
...
🤖 AI 전략:
  - RAG 검색: "음식점 평균 테이블 수", "식사 시간 평균"
  - 물리 법칙: 시간/공간 제약은 보편적
  - 유사 패턴: RAG에서 경험 패턴 검색
  - 없으면: 물리적 제약조건으로 추론
```

### 출처 4: Benchmark (RAG 활용 - 조건부)
```
유사 사례:
  - 업계 벤치마크
  - 경쟁사 데이터
...
AI 프로세스:
  1. RAG 검색: 유사 벤치마크 찾기
  2. GuestimationEngine.check_comparability() 실행
  3. 4대 기준 자동 평가
  4. 채택/기각 명시
  5. 논리적 추론
...
### AI Guestimation 체크리스트

```
출처 2 (보편적 지식):
  ✅ 웹 서치로 최신 데이터 확보
  ✅ 일반 지식으로 대략 값
  ✅ Order of Magnitude면 추정

출처 3 (경험 패턴):
  ✅ RAG 검색 (market_benchmarks)
  ✅ 물리 법칙 (시간, 공간)
  ✅ 합리적 범위 가정
