...
**= Fermi Estimation의 실용적 확장**
- 물리학자 Enrico Fermi가 창안
- 큰 문제를 작은 단위로 분해
- Order of Magnitude (자릿수) 추정

## 🎯 핵심 철학

> **"정확하지 않아도 괜찮다. 하지만 근거 없이 추정하면 안 된다."**

> **"정답보다 추정 과정이 중요하다."** - Fermi

> **"보이지 않는 것을 보이는 모습으로 바꾸는 것이 핵심"**
...
비즈니스 적용:
  - 시장 규모: "약 500억" (정확히 487억이 아니어도 OK)
  - 전환율: "약 15%" (정확히 14.7%가 아니어도 OK)
  - 고객 수: "약 10만명" (정확히 98,234명이 아니어도 OK)
```
...
### ✅ 예시 4: Decomposition + Constraint (전봇대) - Fermi 고전!
...
Step 5 지역별 조정 (인구밀도 반영):
  도시 (서울+6대 광역시):
    - 면적: 6,000 km²
    - 인구: 2,237만 (46%)
    - 밀도: 2배
    - 전봇대: 6,000 × 200 × 2 = 240만
...
### 원칙 5: Order of Magnitude (Fermi 원칙)
...
경계:
  - 10만 vs 100만 vs 1,000만
  - 각 단계는 명확히 다름
```

### 원칙 6: 보이지 않는 것을 보이게 (Fermi 철학)
...
예시:
  - "전봇대가 몇 개?" → "면적 × 밀도" 모델
  - "자장면 몇 그릇?" → "중국집 × 용량 × 시간" 모델
  - "여행 지출?" → "인원 × 1인당" 모델

핵심:
  - 논리적 모델 = 보이지 않는 것을 보게 만드는 도구
  - 추정 과정 > 정답
```
