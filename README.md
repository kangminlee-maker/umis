...
```
umis-main/
├── umis_guidelines_v6.2.yaml                # 메인 시스템 정의 (5,004줄)
├── umis_ai_guide_v6.2.yaml                 # AI 사용 가이드 (848줄, AI 최적화)
├── umis_examples_v6.2.yaml                  # 실제 사용 예시 (745줄)
├── umis_business_model_patterns_v6.2.yaml   # 사업모델 패턴 라이브러리 (985줄) 🆕
├── umis_disruption_patterns_v6.2.yaml       # 추월 전략 패턴 라이브러리 (1,912줄) 🆕
├── VERSION.txt                              # 현재 버전: 6.2.2
...
- **모델 적응형 토큰 관리** (v6.2.1 신규) 🆕
  - 컨텍스트 윈도우 자동 감지 (Large/Medium/Small)
  - 모델별 최적 계수 자동 선택 (0.60-0.85)
  - 큰 모델(1M): 효율 극대화, 작은 모델(200K): 안전성 확보
...
- **동적 토큰 관리 시스템** (v6.2.1 신규) 🆕
  - 모델별 자동 적응형 계수 (Large: 0.75-0.85, Medium: 0.65-0.75, Small: 0.60-0.70)
  - 컨텍스트 윈도우 크기 자동 감지 및 최적 계수 선택
  - 3단계 안전장치 (70% 경고, 95% 예측 차단, 98% 긴급)
  - 큰 모델은 효율 극대화, 작은 모델은 안전성 확보
- **Stewart 모니터링 강화**
  - 작업리스트 관리 모니터링 추가
  - 토큰 사용량 실시간 추적
  - 적응적 개입 프로토콜
...
- **권장 AI 모델** (v6.2.1 - 모델 적응형):
  ...
  - **양호**: GPT-5 (272K) ⭐ 
    - 계수: 0.65-0.75 (Medium Model, 자동 적용)
    - 세션당: 1-2개 쿼리, Standard/Quick 권장
  - **제한**: Claude 200K ⚠️ 
    - 계수: 0.60-0.70 (Small Model, 자동 적용)
    - 세션당: 1개 쿼리, Quick Mode만
